...
import { calculateThreeShotGeometry, type Point } from "./features/trim-balance/calculations/geometry";
import { buildSelectedBalancePoints, calculateSelectedPairwiseAnalysis, calculateSelectedResultMetrics, selectedResultantsFromPersisted, storeSelectedResultant, toPersistedResultant, type SelectedResultants } from "./features/trim-balance/calculations/selectedResultants";
...
function App() {
  ...
  const start = (item = createCalculation()) => { setCalculation(item); setSelectedResultants(selectedResultantsFromPersisted(item.selectedResultants)); setStep(0); };
  const selectResultant = (point: Point) => {
    const label = active?.point.label ?? selected;
    setSelectedResultants((items) => storeSelectedResultant(items, label, point));
    const metrics = active ? calculateSelectedResultMetrics(active.point, point) : undefined;
    if (metrics) update((item) => ({ ...item, selectedResultants: { ...item.selectedResultants, [label]: toPersistedResultant(label, metrics) } }));
  };

  if (!calculation) return <Dashboard saved={saved} onStart={start} />;
  const page = renderStep({ step, calculation, update, updateMeasurement, errors, points, analyses, active, selected, setSelected, selectedResultants, selectResultant, pairs, finalCorrection, selectedMetrics });
...
function renderStep({ step, calculation, update, updateMeasurement, errors, points, analyses, active, selected, setSelected, selectedResultants, selectResultant, pairs, finalCorrection, selectedMetrics }: { step: number; calculation: TrimBalanceCalculation; update: UpdateCalculation; updateMeasurement: (run: RunName, speed: N1Speed, field: "bearingVibration" | "ffccvVibration", value: number) => void; errors: string[]; points: ReturnType<typeof buildPlotPoints>; analyses: { point: PlotPoint; geometry: ReturnType<typeof calculateThreeShotGeometry> }[]; active: { point: PlotPoint; geometry: ReturnType<typeof calculateThreeShotGeometry> } | undefined; selected: PlotPointLabel; setSelected: (label: PlotPointLabel) => void; selectedResultants: SelectedResultants; selectResultant: (point: Point) => void; pairs: ReturnType<typeof calculateSelectedPairwiseAnalysis>; finalCorrection: FinalCorrectionResult | undefined; selectedMetrics: (point: PlotPoint) => ResultantMetrics | undefined }) {
  ...
  if (step === 6) return <PlotAnalysis calculation={calculation} update={update} errors={errors} analyses={analyses} active={active} selected={selected} onSelect={setSelected} selectedResultants={selectedResultants} onSelectResultant={selectResultant} getMetrics={selectedMetrics} />;
